...
class _SignUpBodyState extends State<SignUpBody> {
                  ...
                  child: UploadFileWidget(
                      btnLebal: _cnicFrontImage == null
                          ...
                          : _cnicFrontImage!.path
                              ...
                              .toString()
                              .replaceAll('scaled_', ''),
                      fileName: 'Upload CNIC Front Image'.tr()),
                ),
                const SizedBox(
                  height: 18,
                ),
                  ...
                  child: UploadFileWidget(
                      btnLebal: _cnicBackImage == null
                          ...
                          : _cnicBackImage!.path
                              ...
                              .toString()
                              .replaceAll('scaled_', ''),
                      fileName: 'Upload CNIC Back Image'.tr()),
                ),
                const SizedBox(
                  height: 18,
                ),
                ...
                AppButton(
                    onPressed: () {
                      ...
                      if (_cnicController.text.isEmpty) {
                        getFlushBar(context,
                            title: 'CNIC cannot be empty.'.tr());
                        return;
                      }
                      if (_cnicController.text.length != 13) {
                        getFlushBar(context,
                            title: 'Kindly enter valid CNIC.'.tr());
                        return;
                      }
                      ...
                      if (_cnicFrontImage == null) {
                        getFlushBar(context,
                            title: "Kindly attach your CNIC Front image.".tr());
                        return;
                      }
                      if (_cnicBackImage == null) {
                        getFlushBar(context,
                            title: "Kindly attach your CNIC Back image.".tr());
                        return;
                      }
  ...
  _signUpUser({
    ...
    await signUp
      ...
      userModel: RiderModel(
          ...
          cityName: _selectedCity!.name.toString(),
          isApproved: false,
      ...
      if (signUp.status == SignUpStatus.Registered) {
        showNavigationDialog(context,
            message: "Thanks for registration on our system.".tr(),
            buttonText: "Go to Next".tr(), navigation: () {
          Navigator.push(
              context,
              MaterialPageRoute(
  ...
  Future getCNICFrontImage() async {
    XFile? res = await ImagePicker()
        .pickImage(source: ImageSource.gallery, imageQuality: 50);
    setState(() {
      if (res != null) {
        _cnicFrontImage = File(res.path);
      } else {
  ...
  Future getCNICBackImage() async {
    XFile? res = await ImagePicker()
        .pickImage(source: ImageSource.gallery, imageQuality: 50);
    setState(() {
      if (res != null) {
        _cnicBackImage = File(res.path);
      } else {
        print('No image selected.');
      }
    });
